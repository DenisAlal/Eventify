...
class RichInput extends TextField {
  const RichInput({
    Key? key,
    required RichInputController controller,
...
class RichInputController extends TextEditingController {
  ...
  TextEditingValue _formatValue(
    ...
    if (delLength == 1 || delLength == 2) {
      final offset = newValue.selection.baseOffset;
      final deletedText = oldText.substring(offset, offset + delLength);

      if (deletedText.contains(_specialChar)) {
  ...
  //   if (oldValue == newValue || newValue.text.length >= oldValue.text.length || newValue.selection.baseOffset == -1) {
  //     return newValue;
  //   }
  //   final oldText = oldValue.text;
  //   final delLength = oldText.length - newValue.text.length;
  ...
  //   late int offset;
  //   if (delLength == 1) {
  //     char = oldText.substring(newValue.selection.baseOffset, newValue.selection.baseOffset + 1);
  //     offset = newValue.selection.baseOffset;
  //   } else if (delLength == 2) {
  //     // two characters will be deleted on huawei
  //     char = oldText.substring(newValue.selection.baseOffset + 1, newValue.selection.baseOffset + 2);
  ...
  //     }
  //     if (str != delStr && str != oldStr) {
  //       str += newValue.selection.textInside(newText) + newValue.selection.textAfter(newText);
  //
  //       final len = newText.length - str.length;
  //       return newValue.copyWith(
  //         text: str,
